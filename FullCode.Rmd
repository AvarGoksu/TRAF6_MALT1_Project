...
```{r}
Ig_genes <- cnt[grep("^Ig", rownames(cnt)),]
Ig_genes <- as.data.frame(Ig_genes)
```

```{r}
View(subset(coding_genes, (rownames(coding_genes) %in% rownames(Ig_genes))))
```
...
```{r}
Igsf_genes <- cnt[grep("^Igsf", rownames(cnt)),]
Igsf_genes <- as.data.frame(Igsf_genes)
```

```{r}
Igll_genes <- cnt[grep("^Igll", rownames(cnt)),]
Igll_genes <- as.data.frame(Igll_genes)
```

```{r}
Igdcc_genes <- cnt[grep("^Igdcc", rownames(cnt)),]
Igdcc_genes <- as.data.frame(Igdcc_genes)
```

```{r}
Genes_to_filterout <- bind_rows(Igh_genes, Igk_genes, Igsf_genes, Igll_genes, Igdcc_genes)
```
...
```{r}
cnt <- cnt %>% dplyr::select(-Sample_MUC26954)
cnt.filt <- cnt.filt[,-11]
coldata <- coldata[-11,]
```

```{r}
order <- c("Sample_MUC26944","Sample_MUC26945","Sample_MUC26951","Sample_MUC26952","Sample_MUC26953", "Sample_MUC26950", "Sample_MUC26956", "Sample_MUC26957", "Sample_MUC26958", "Sample_MUC26946", "Sample_MUC26947", "Sample_MUC26948", "Sample_MUC26949", "Sample_MUC26955")
...
ggplot(pcaData, aes(PC1, PC2, color=Genotype, shape=Sex)) +
  ...
  geom_encircle(data=subset(pcaData, Genotype=="TBMT"))+
  geom_encircle(data=subset(pcaData, Genotype=="Traf6dT"), expand=0.08)+
  geom_encircle(data=subset(pcaData, Genotype=="Control"))+
  coord_fixed()
dev.off()
```
```{r}
...
biplot(p, 
       colby = "Genotype", 
       colkey = c('TBMT' = 'red', 'Traf6dT' = 'blue', "Control" = "purple"), 
       lab=NA,
       pointSize = 6,
...
```{r}
hist(res_traf6dt_ctrl_sig_sort_p$padj, main="Traf6dT vs Control (Filtered)", xlab="padj-values",cex.main=1.5, breaks = 10)

hist(res_tbmt_ctrl_sig_sort_p$padj, main="TBMT vs Control (Filtered)", xlab="padj-values",cex.main=1.5, breaks = 10)

hist(res_tbmt_traf6dt_sig_sort_p$padj, main="TBMT vs Traf6dT (Filtered)", xlab="padj-values",cex.main=1.5, breaks = 10)
...
df <- as.data.frame(as.data.frame(coldata)$Genotype)
rownames(df) <- row.names(coldata)
colnames(df) <- c("Genotype")
png("Heatmap.png", width=1280, height = 1440)
pheatmap(subset(vsdMat, rownames(vsdMat) %in% sig_list_combined), cluster_rows=TRUE, show_rownames=TRUE,
...
y_tbmt <- gsePathway(sort((tbmt_lfc), decreasing=T), organism = "mouse", pvalueCutoff=0.1, pAdjustMethod="BH", verbose=F)

tbmt_gsea <- y_tbmt@result
tbmt_gsea <- tbmt_gsea %>% arrange(desc(NES))

gseaplot(y_tbmt, geneSetID = tbmt_gsea$ID[2], title=tbmt_gsea$Description[2])

#viewPathway("TNFR2 non-canonical NF-kB pathway", readable=T, organism="mouse", foldChange = sort((tbmt_lfc), decreasing=T))

mouse_human_orthologs <- read.csv("HOM_MouseHumanSequence.tsv", sep="\t")

mouse_entrez <- mouse_human_orthologs %>%
  dplyr::filter(Common.Organism.Name == "mouse, laboratory") %>%
  dplyr::select(DB.Class.Key, Common.Organism.Name, Symbol, EntrezGene.ID)

human_entrez <- mouse_human_orthologs %>%
  dplyr::filter(Common.Organism.Name == "human") %>%
  dplyr::select(DB.Class.Key, Common.Organism.Name, Symbol, EntrezGene.ID)

mouse_human_entrez <- left_join(mouse_entrez, human_entrez, by="DB.Class.Key")

human_sg_df <- mouse_human_entrez %>% 
  dplyr::filter(EntrezGene.ID.x %in% sg) %>%
  dplyr::select(EntrezGene.ID.y)

human_sg <- human_sg_df$EntrezGene.ID.y

x_tbmt_up_dose <- enrichDO(gene=human_sg, ont="DO", pvalueCutoff=0.1, pAdjustMethod="BH", readable=T)
```
...
y_traf6dt <- gsePathway(sort((traf6dt_lfc), decreasing=T), organism = "mouse", pvalueCutoff=0.1, pAdjustMethod="BH", verbose=F)

traf6dt_gsea <- y_traf6dt@result
traf6dt_gsea <- traf6dt_gsea %>% arrange(desc(NES))

gseaplot(y_traf6dt, geneSetID = traf6dt_gsea$ID[2], title=traf6dt_gsea$Description[2])
...
x_traf6dt_up_dose <- enrichDO(gene=human_sg, ont="DO", pvalueCutoff=0.1, pAdjustMethod="BH", readable=T)

dose_traf6dt_up <- x_traf6dt_up_dose@result
```
...
dotplot(x_tbmt_traf6dt_down, showCategory=30, font.size=15, label_format = 10) + ggtitle("TBMT vs Traf6dT Direct Comparison (Upregulated\nin Traf6dT)") + theme(plot.title = element_text(size = 30, face = "bold"))
dev.off()
...
ggplot(Reactome_commonUp, aes(x=(-log10(qvalue.x)), y=(-log10(qvalue.y))))+
  ...
  theme(axis.title = element_text(size = 40, face = "bold"), axis.text=element_text(size=20, face = "bold"))+
  geom_label_repel(data=subset(Reactome_commonUp, Significance== "Both"), aes(label=Description.x), size=7, nudge_y=0.15, nudge_x=2, color="black", fontface="bold", min.segment.length = unit(0, "lines"), max.overlaps = 7.5, angle=45)+
...
ggplot(Reactome_commonDown, aes(x=(-log10(qvalue.x)), y=(-log10(qvalue.y))))+
  ...
  ylab("TBMT vs Control (-log10(p-adj))")+
  theme(axis.title = element_text(size = 30, face = "bold"), axis.text=element_text(size=20, face = "bold"))+
  geom_label_repel(data=subset(Reactome_commonDown, Significance == "Both" | Significance == "Only TBMT" | Significance == "Only Traf6dT"), aes(label=Description.x), size=7, color="black", fontface="bold", nudge_y = 0.1, min.segment.length = unit(0, "lines"), max.overlaps = Inf)+
  geom_point(aes(x=4, y=1), color="purple", size=20)+
  geom_point(aes(x=4, y=0.95), color="red", size=20)+
  geom_point(aes(x=4, y=0.9), color="blue", size=20)+
  annotate("text", x=4.1, y=1, label="Both", size=11, hjust=0)+
  annotate("text", x=4.1, y=0.95, label="Only TBMT", size=11, hjust=0)+
  annotate("text", x=4.1, y=0.9, label="Only Traf6dT", size=11, hjust=0)+
  annotate("text", x=3.9, y=1.05, label="Significance (p-adj<0.1)", size=13, fontface="bold.italic", hjust=0)
dev.off()
```
...
ggplot(tbmt_traf6dt_list, aes(x=log2FoldChange, y=(minuslogp)))+
  ...
  geom_vline(xintercept = -1, linetype="dashed")+
  geom_hline(yintercept = -log10(0.1), linetype="dashed")+
  geom_label_repel(data=subset(tbmt_traf6dt_list, minuslogp>5 | abs(log2FoldChange)>2), aes(label=gene), size=6, color="black", nudge_y = 0.5, fontface="bold",  min.segment.length = unit(0, "lines"), max.overlaps = 20)+
...
ggplot(tbmt_ctrl_list, aes(x=log2FoldChange, y=(minuslogp)))+
  ...
  geom_vline(xintercept = -1, linetype="dashed")+
  geom_hline(yintercept = -log10(0.1), linetype="dashed")+
  geom_label_repel(data=subset(tbmt_ctrl_list, minuslogp>10 | abs(log2FoldChange)>2.5), aes(label=gene), size=6, color="black", nudge_y = 0.5, fontface="bold",  min.segment.length = unit(0, "lines"), max.overlaps = 20)+
  geom_point(aes(x=-3.3, y=110), color="red3", size=20)+
  geom_point(aes(x=-3.3, y=104), color="darkorange1", size=20)+
  geom_point(aes(x=-3.3, y=98), color="yellowgreen", size=20)+
  annotate("text", x=-3.05, y=110.05, label="p-adj<0.1 |LFC|>1", size=12, hjust=0)+
  annotate("text", x=-3.05, y=104.05, label="p-adj<0.1 |LFC|<1", size=12, hjust=0)+
  annotate("text", x=-3.05, y=98.05, label="p-adj>0.1 |LFC|>1", size=12, hjust=0)
dev.off()
...
ggplot(traf6dt_ctrl_list, aes(x=log2FoldChange, y=(minuslogp)))+
  ...
  geom_vline(xintercept = -1, linetype="dashed")+
  geom_hline(yintercept = -log10(0.1), linetype="dashed")+
  geom_label_repel(data=subset(traf6dt_ctrl_list, minuslogp>5 | abs(log2FoldChange)>2), aes(label=gene), size=6, color="black", nudge_y = 0.5, fontface="bold",  min.segment.length = unit(0, "lines"), max.overlaps = 20)+
  geom_point(aes(x=-3, y=20), color="red3", size=20)+
  geom_point(aes(x=-3, y=19), color="darkorange1", size=20)+
  geom_point(aes(x=-3, y=18), color="yellowgreen", size=20)+
  annotate("text", x=-2.85, y=20.05, label="p-adj<0.1 |LFC|>1", size=12, hjust=0)+
  annotate("text", x=-2.85, y=19.05, label="p-adj<0.1 |LFC|<1", size=12, hjust=0)+
  annotate("text", x=-2.85, y=18.05, label="p-adj>0.1 |LFC|>1", size=12, hjust=0)
dev.off()
...
cnt.tib.new <- tpm3 %>% dplyr::select(GeneID,sample,tpm) %>% pivot_wider(names_from = sample,
values_from = tpm)
...
order_deconv <- c("cell_type", "Sample_MUC26944","Sample_MUC26945","Sample_MUC26951","Sample_MUC26952","Sample_MUC26953", "Sample_MUC26950", "Sample_MUC26956", "Sample_MUC26957", "Sample_MUC26958", "Sample_MUC26946", "Sample_MUC26947", "Sample_MUC26948", "Sample_MUC26949", "Sample_MUC26955")
...
newmatrixCounts_traf6dt <- newmatrixCounts[,6:9]
newmatrixCounts_ctrl <- newmatrixCounts[,10:14]
```
...
epic %>%
  ...
  ggplot(aes(x=sample, y=fraction, fill=cell_type)) +
    ...
    labs(y="Fraction of the cell type", x="Samples", title="EPIC Deconvolution")+
    guides(fill=guide_legend(title="Cell Type"))+
    theme(axis.text = element_text(size=12), axis.title = element_text(size=14, face ="bold"), plot.title = element_text(size=16, face ="bold", hjust = 0.5), legend.title = element_text(size=11, face ="bold"))+
    coord_flip()

write.xlsx(epic, "epic.xlsx", overwrite=TRUE)
```

```{r}
TBMT_samples <- c("Sample_MUC26944", "Sample_MUC26945", "Sample_MUC26951", "Sample_MUC26952", "Sample_MUC26953")
Traf6dt_samples <- c("Sample_MUC26950", "Sample_MUC26956", "Sample_MUC26957", "Sample_MUC26958")
Ctrl_samples <- c("Sample_MUC26946", "Sample_MUC26947", "Sample_MUC26948", "Sample_MUC26949", "Sample_MUC26955")

epic_melt <- melt(epic)

epic_melt <- epic_melt %>% 
  mutate(Genotype = ifelse(epic_melt$variable %in% TBMT_samples, "TBMT", ifelse(epic_melt$variable %in% Traf6dt_samples, "Traf6dT", "Control")))

ggplot(data = subset(epic_melt,cell_type=="T cell CD4+" | cell_type=="T cell CD8+"), aes(x=Genotype, y=value))+
  geom_boxplot(aes(fill=cell_type), alpha=0.7, outlier.shape=NA)+
  geom_point(position = position_jitter(width = 0.1, height = 0))+
  theme(panel.background = element_blank(),plot.background = element_blank(), axis.line = element_line()) +
  stat_compare_means(method = "t.test")+
  facet_wrap(~cell_type)
```

```{r}
compare_means(value ~ Genotype, data = subset(epic_melt,cell_type=="T cell CD4+" | cell_type=="T cell CD8+"), 
              group.by = "cell_type")

my_comparison <- list(c("TBMT", "Control"), c("TBMT", "Traf6dT"), c("Control", "Traf6dT") )

#png("T_cell_fractions.png", width=800, height=600)
ggboxplot(data = subset(epic_melt,cell_type=="T cell CD4+" | cell_type=="T cell CD8+"), x = "Genotype", y = "value",
          color = "Genotype", palette = "npg", facet.by = "cell_type", outlier.shape = NA, add = "jitter") + stat_compare_means(comparisons = my_comparison, method = "t.test") + ylab("Cell Fraction") + guides(fill=guide_legend(title="Cell Type"))+ theme(axis.title = element_text(size=14, face ="bold"))
#dev.off()
...
for(i in 1:nrow(Reactome_commonUp_Excel)){
  ID <- Reactome_commonUp_Excel$ID[i]
  Description <- Reactome_commonUp_Excel$Description[i]
  Significance <- as.character(Reactome_commonUp_Excel$Significance[i])
  List_Traf6dT <- unlist(strsplit(Reactome_commonUp_Excel$`List of upregulated genes in this pathway (Traf6dT vs Control)`[i], "/"))
  List_TBMT <- unlist(strsplit(Reactome_commonUp_Excel$`List of upregulated genes in this pathway (TBMT vs Control)`[i], "/"))

  if(Significance == "Only TBMT"){ 
    png(file=paste0("TBMTvsCtrl_Volcano",ID,".png"), width=1980, height=1440)
    print(ggplot(tbmt_ctrl_list, aes(x=log2FoldChange, y=(minuslogp)))+
      geom_point(size=2.25,color="grey", alpha=0.4)+
      geom_point(data=subset(tbmt_ctrl_list, tbmt_ctrl_list$gene %in% List_TBMT), color="red3", alpha=1, size=5)+
      theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(),plot.background = element_blank(), axis.line = element_line()) +
      ggtitle("TBMT vs Control")+
      theme(plot.title = element_text(size = 60, face = "bold"))+
      ...
      geom_vline(xintercept = -1, linetype="dashed")+
      geom_hline(yintercept = -log10(0.1), linetype="dashed")+
      geom_label_repel(data=subset(tbmt_ctrl_list, tbmt_ctrl_list$gene %in% List_TBMT & (tbmt_ctrl_list$padj < 0.1 & tbmt_ctrl_list$log2FoldChange > 1)), aes(label=gene), size=7.5, color="black", nudge_y = 0.05, fontface = "bold", min.segment.length = unit(0.3, "lines"), max.overlaps = Inf)+
      annotate("text", x=-3, y=100, label=paste0(Description), size=12, hjust=0, color="red3")+
      annotate("text", x=-3, y=94, label=paste0(ID), size=12, hjust=0, color="red3"))
    dev.off()}
    if(Significance=="Only Traf6dT"){
      png(file=paste0("Traf6dTvsCtrl_Volcano", ID, ".png"), width=1980, height=1440)
      print(ggplot(traf6dt_ctrl_list, aes(x=log2FoldChange, y=(minuslogp)))+
        geom_point(size=2.25,color="grey", alpha=0.4)+
        geom_point(data=subset(traf6dt_ctrl_list, traf6dt_ctrl_list$gene %in% List_Traf6dT), color="red3", alpha=1, size=5)+
        theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        ...
        geom_vline(xintercept = -1, linetype="dashed")+
        geom_hline(yintercept = -log10(0.1), linetype="dashed")+
        geom_label_repel(data=subset(traf6dt_ctrl_list, traf6dt_ctrl_list$gene %in% List_Traf6dT & (traf6dt_ctrl_list$padj < 0.1 & traf6dt_ctrl_list$log2FoldChange > 1)), aes(label=gene), size=7.5, color="black", nudge_y = 0.05, fontface = "bold", min.segment.length = unit(0.3, "lines"), max.overlaps = Inf)+
        annotate("text", x=-3, y=20, label=paste0(Description), size=12, hjust=0,
                 color="red3")+
        annotate("text", x=-3, y=18, label=paste0(ID), size=12, hjust=0, color="red3"))
    dev.off()}
      if(Significance=="Both"){
        plot1 <- ggplot(tbmt_ctrl_list, aes(x=log2FoldChange, y=(minuslogp)))+
      geom_point(size=1,color="grey", alpha=0.4)+
      geom_point(data=subset(tbmt_ctrl_list, tbmt_ctrl_list$gene %in% List_TBMT), color="red3", alpha=1, size=1)+
      theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(),plot.background = element_blank(), axis.line = element_line()) +
      ggtitle("TBMT vs Control")+
      #theme(plot.title = element_text(size = 60, face = "bold"))+
      ...
      geom_vline(xintercept = -1, linetype="dashed")+
      geom_hline(yintercept = -log10(0.1), linetype="dashed")+
      geom_label_repel(data=subset(tbmt_ctrl_list, tbmt_ctrl_list$gene %in% List_TBMT & (tbmt_ctrl_list$padj < 0.01 & tbmt_ctrl_list$log2FoldChange > 1)), aes(label=gene), size=2, color="black", nudge_y = 0.05, fontface = "bold", min.segment.length = unit(0.3, "lines"), max.overlaps = Inf)+
      annotate("text", x=-3, y=100, label=paste0(Description), size=2, hjust=0, color="red3")+
      annotate("text", x=-3, y=94, label=paste0(ID), size=2, hjust=0, color="red3")
        plot2 <- ggplot(traf6dt_ctrl_list, aes(x=log2FoldChange, y=(minuslogp)))+
        geom_point(size=1,color="grey", alpha=0.4)+
        geom_point(data=subset(traf6dt_ctrl_list, traf6dt_ctrl_list$gene %in% List_Traf6dT), color="red3", alpha=1, size=1)+
        theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
              panel.background = element_blank(),plot.background = element_blank(),
              axis.line = element_line()) +
        ggtitle("Traf6dT vs Control")+
       ...
       # theme(axis.title = element_text(size = 40, face = "bold"),
        ...
        geom_vline(xintercept = -1, linetype="dashed")+
        geom_hline(yintercept = -log10(0.1), linetype="dashed")+
        geom_label_repel(data=subset(traf6dt_ctrl_list, traf6dt_ctrl_list$gene %in% List_Traf6dT & (traf6dt_ctrl_list$padj < 0.01 & traf6dt_ctrl_list$log2FoldChange > 1 )), aes(label=gene), size=2, color="black", nudge_y = 0.05, fontface = "bold", min.segment.length = unit(0.3, "lines"), max.overlaps = Inf)+
        annotate("text", x=-3, y=20, label=paste0(Description), size=2, hjust=0,
                 color="red3")+
        annotate("text", x=-3, y=18, label=paste0(ID), size=2, hjust=0, color="red3")
        p <- plot_grid(plot1, plot2)
        ggsave(paste0(ID, "plot.pdf"), p)
      }} 
```
...
for(i in 1:nrow(Reactome_commonUp_Excel)){
  ...
  List_Traf6dT <- unlist(strsplit(Reactome_commonUp_Excel$`List of upregulated genes in this pathway (Traf6dT vs Control)`[i], "/"))
  List_TBMT <- unlist(strsplit(Reactome_commonUp_Excel$`List of upregulated genes in this pathway (TBMT vs Control)`[i], "/"))
  Common_List <- unique(c(List_Traf6dT, List_TBMT))
  
  if(Significance == "Only TBMT"){
    print((Common_List))
    print(subset(tbmt_ctrl_list, c(tbmt_ctrl_list$gene) %in% c(Common_List)))}}
